...
const parseGuild = (message) => {
    const regex = /��\*0x\w+��(\w+)��/g;
    const guildString = message.match(regex);
    if (guildString) {
        const guildName = guildString[0].split("��")[2];
        return guildName;
    }
    return "";
};
...
const getNextChopTime = (lastTimer) => {
    const nextChopTime = lastTimer + (2 * 60 * 60 * 1000) + (30 * 60 * 1000);
    return nextChopTime;
};
...
async function getWs (session, landNumber) {
    try {
        ...
        const landNumberStr = landNumber.landNumber;
        return {trees: messagesList, land: landNumberStr, guild};
    } catch (error) {
        console.log('Erro ao processar mensagem:', error);
        return;
    }
